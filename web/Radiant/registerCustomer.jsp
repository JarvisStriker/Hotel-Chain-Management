...
ResultSet rs=st.executeQuery("select * from customer where hotel like 'Radiant' and room_no="+rn+";");
while(rs.next())
    error++;
if(error==0)
{
    int i=st.executeUpdate("insert into customer values ('" + customerRoomNo + "','Radiant','" + customerName + "','" + customerAadhaar + "','" + customerEmail + "','" + customerPhone + "','" + customerGender + "',CURDATE())");
    int j=st.executeUpdate("insert into rooms_radiant(room_no,room_type) values('"+customerRoomNo+"','"+customerRoomType+"')");
