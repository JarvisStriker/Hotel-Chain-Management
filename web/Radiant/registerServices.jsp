...
ResultSet rs=st.executeQuery("select * from rooms_radiant where room_no="+room_no+";"); 
while(rs.next()){
    i=rs.getInt("amt");
    i+=total;    
}
int j1=st.executeUpdate("insert into services values("+room_no+",'Radiant','"+services+"',"+total+");");
int j=st.executeUpdate("update rooms_radiant set amt="+i+" where room_no="+room_no+";"); 
%>
